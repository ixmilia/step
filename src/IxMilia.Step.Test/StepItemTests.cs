...
";
        ...
        private StepItem ReadTopLevelItem(string data)
        {
            var file = ReadFile(data);
            return file.GetTopLevelItems().Single();
        }

        private void AssertEquivalent<T>(T expected, T actual)
        {
        ...
        private void AssertFileContains(StepItem item, string expected)
        {
...
");
            ...
            // only LINE() isn't referenced by another item
            var _line = (StepLine)file.GetTopLevelItems().Single();
        }

        [Fact]
        public void ReadTopLevelInlinedItemsTest()
        {
            var file = ReadFile("#1=LINE('',CARTESIAN_POINT('',(1.0,2.0,3.0)),VECTOR('',DIRECTION('',(0.0,0.0,1.0)),15.0));");
